...
var tot,tot2,tot3,n,hang,lie,i,j,le,ri,nx,ny,tmp,total,total2,cnt,p,ans,x:longint;
    l,r:longint;
    a,ra,ca:array[0..maxn]of rec;
    stx,enx:array[0..maxcoordinate]of longint;
    vis:array[0..maxn]of boolean;
    topsort,topsort2,col,inner,sum,f,outer,edge,edge2,edge3,hash:array[0..maxn]of longint;
...
while p<>0 do
  begin
  if hash[h[p].v]=0 then dfs3(h[p].v);
  p:=h[p].nxt;
  end;
inc(total2);
topsort2[total2]:=x;
...
for i:=1 to cnt do
    if hash[i]=0 then
       dfs3(i);
for i:=total2 downto 1 do
    begin
    p:=edge3[topsort2[i]];
    while p<>0 do
      begin
      if f[topsort2[i]]+sum[h[p].v]>f[h[p].v] then
         f[h[p].v]:=f[topsort2[i]]+sum[h[p].v];
      p:=h[p].nxt;
      end;
    end;
for i:=1 to cnt do
