//uses dos;
const maxn=1000020; maxm=120020; maxbina=65;
...
var r,c,i,j,n,m,avoid,tot,now,ans,point,cnt:longint;
    ...
    ori,bel,axis:array[0..maxn]of longint;
...
if l=r then
   ...
   for i:=1 to tot do
       ...
       if con[i][1]=hash[avoid] then tree[x].mat[i]:=(rr-ll+1+1)div 2
          else if i=hash[avoid] then tree[x].mat[i]:=(rr-ll+1)div 2
                  else tree[x].mat[i]:=0;
       end;
   exit;
   end;
...
if tag[x]<>0 then
   ...
   for i:=1 to tot do
       begin
//       inc(cnt);
       ll:=ori[l]; tm:=ori[mid+1]-1; rr:=ori[r+1]-1;
       if (tm-ll+1)mod 2=1 then tree[x*2].ret[i]:=con[i][tag[x]] else tree[x*2].ret[i]:=i;
       if con[i][tag[x]]=hash[avoid] then
          begin
          tree[x*2].mat[i]:=(tm-ll+1+1)div 2;
          tree[x*2+1].mat[i]:=(rr-tm+1)div 2;
          end
       else
          begin
          if i=hash[avoid] then begin tree[x*2].mat[i]:=(tm-ll+1)div 2; tree[x*2+1].mat[i]:=(rr-tm)div 2; end
             else begin tree[x*2].mat[i]:=0; tree[x*2+1].mat[i]:=0; end;
          end;
       if (rr-tm)mod 2=1 then tree[x*2+1].ret[i]:=con[i][tag[x]] else tree[x*2+1].ret[i]:=i;
       end;
   tag[x]:=0;
   end;
end;
...
//       inc(cnt);
       ...
       if (rr-ll+1)mod 2=1 then tree[x].ret[i]:=con[i][v] else tree[x].ret[i]:=i;
       if con[i][v]=hash[avoid] then tree[x].mat[i]:=(rr-ll+2)div 2
          else if i=hash[avoid] then tree[x].mat[i]:=(rr-ll+1)div 2
                  else tree[x].mat[i]:=0;
       end;
   exit;
