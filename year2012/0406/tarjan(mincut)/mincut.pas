...
var n,m,s,t,i,x,y,z,sum,tot,top,cnt,tme:longint;
    h,hash,edge,dfn,low,col,stack:array[0..maxn]of longint;
...
while p<>0 do
  ...
  if (g[p].c>0) then
     ...
     else if (instack[g[p].v])and(dfn[g[p].v]<low[x]) then
             low[x]:=dfn[g[p].v];
     end;
  p:=g[p].nxt;
  end;
