...
var n,i,cnt,j,k,ans,l,fmax,pd:longint;
    a,b,aa,bb,sta,lll,rrr:array[0..2*maxn]of longint;
    seg:array[0..2*maxn]of segtype;
    f,g:array[0..maxn,0..maxn,0..2*maxn]of longint;
...
reset(input);
assign(output,'e:\wqf2\show2.out');
rewrite(output);
readln(n);
for i:=1 to n do
    begin
   ...
   for i:=1 to n do
       for j:=0 to i do
           ...
           for l:=0 to i-1 do
               ...
               for k:=0 to cnt do
                   begin
                   if a[k]>a[seg[i].ll] then break;
                   if g[l][j-1][k]>f[i][j][seg[i].rr] then f[i][j][seg[i].rr]:=g[l][j-1][k];
                   end;
               end;
           end;
           for l:=0 to i-1 do
               ...
               for k:=0 to cnt do
                   begin
                   if a[k]>a[seg[i].ll] then break;
                   if f[l][j][k]+1>g[i][j][seg[i].rr] then g[i][j][seg[i].rr]:=f[l][j][k]+1;
                   end;
               end;
           for l:=0 to i-1 do
               begin
               for k:=0 to cnt do
                   begin
                   if g[l][j][k]=-inf then continue;
                   fmax:=seg[i].rr; if a[k]>a[seg[i].rr] then fmax:=k;
                   if g[l][j][k]+1>g[i][j][fmax] then g[i][j][fmax]:=g[l][j][k]+1;
                   end;
               end;
           if j>=1 then
              begin
              for l:=0 to i-1 do
                  ...
                  for k:=0 to cnt do
                      begin
                      if f[l][j-1][k]=-inf then continue;
                      fmax:=seg[i].rr; if a[k]>a[seg[i].rr] then fmax:=k;
                      if f[l][j-1][k]>f[i][j][fmax] then f[i][j][fmax]:=f[l][j-1][k];
                      end;
                  end;
              end;
           end;
